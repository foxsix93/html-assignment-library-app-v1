...
async function handleClickEditButton(bookId) {
  try {
    ...
    // Simpan hasil fetch ke dalam currentBook
...
async function handleClickDeleteButton(bookId) {
  try {
    ...
    // TODO: answer here
    // const confirmasi = confirm('Apakah anda yakin ingin menghapus buku ini?');
    // if (!confirmasi) {
    //   return;
    // }
  ...
  try {
    ...
    */
    ...
    const book = {
      ...
      year: parseInt(form.elements[2].value, 10),
      quantity: parseInt(form.elements[3].value, 10),
    }
...
async function handleAddForm(event) {
  try {
    ...
    */
    ...
    console.log("BOOK handleAddForm: ", book)
// batas pengerjaan
...
async function deleteBook(bookId) {
  try {
    ...
    */
    ...
    const data = await response.json();
    console.log('Buku berhasil dihapus',data);
    return data;
    // batas pengerjaan
  } catch (error) {
    console.log(error);
